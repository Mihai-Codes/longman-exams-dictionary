...
async function createMainWindow() {
  ...
  // Two-finger trackpad swipes walk Guide article history (macOS gesture —
  // the renderer subscribes to "guide:swipe"). Fired by the OS, never by pages.
  mainWindow.on("swipe", (_event, direction) => {
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send("guide:swipe", direction);
    }
  });
